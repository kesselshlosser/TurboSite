...
class Projects extends Turbo
{
	...
	/*
	*
	* Функция возвращает проект по его id или url
	* (в зависимости от типа аргумента, int - id, string - url)
	* @param $id id или url проекта
	*
	*/
	...
	/*
	*
	* Функция возвращает массив проектов, удовлетворяющих фильтру
	* @param $filter
	*
	*/
	...
	/*
	*
	* Функция вычисляет количество проектов, удовлетворяющих фильтру
	* @param $filter
	*
	*/
	...
	*/	
	public function delete_project($id)
	{
		if(!empty($id))
		{
			...
			// Удаляем поект из связанных с другими
   ...
   public function get_related_projects($project_id = array())
	...
	// Функция возвращает связанные проекты
	...
	*/	
	public function get_next_project($id)
	{
		...
		$this->db->query("(SELECT id FROM __projects WHERE date=? AND id>? AND visible  ORDER BY id limit 1)
		                   UNION
	...
	*/	
	public function get_prev_project($id)
	{
		...
		$this->db->query("(SELECT id FROM __projects WHERE date=? AND id<? AND visible ORDER BY id DESC limit 1)
		                   UNION
