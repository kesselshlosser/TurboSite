...
class Comments extends Turbo
{
	...
	{
		$query = $this->db->placehold("SELECT c.id, c.object_id, c.name, c.ip, c.type, c.text, c.date, c.approved, c.parent_id, c.admin, c.rate FROM __comments c WHERE id=? LIMIT 1", intval($id));
	...
	{	
		...
		$query = $this->db->placehold("SELECT c.id, c.object_id, c.ip, c.name, c.text, c.type, c.date, c.approved, c.parent_id, c.admin, c.rate
	...
	{	
		...
		$query = $this->db->placehold("SELECT c.id, c.object_id, c.ip, c.name, c.text, c.type, c.date, c.text, c.approved, c.parent_id, c.admin, c.rate
